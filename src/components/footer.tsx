import { MailIcon, Phone } from "lucide-react";

export default function Footer() {
  return(<>
    <section id="footer" className="flex justify-center gap-50 min-h-[10rem] px-20 py-8 bg-[#1d1e1e] font-outfit">
      <div className="flex flex-col gap-5 justify-center items-center">
        <img src="/img/anadolu_white_notext_tight.png" className="size-12" />

        <p className="text-sm">© 2026 drizzy.no. All rights reserved.</p>
      </div>

      <div className="flex flex-col gap-3">
        <h1 className="text-2xl">Contact</h1>

        <div className="flex items-center gap-3">
          <MailIcon size={21} className="text-[#b2b2a8]" />

          <span className="text-[#b2b2a8]">yucecontact@gmail.com</span>
        </div>

        <div className="flex items-center gap-3">
          <Phone size={21} className="text-[#b2b2a8]" />

          <span className="text-[#b2b2a8]">+47 69 69 69 69</span>
        </div>

        <div className="flex items-center gap-2">
          <a href="https://www.linkedin.com/in/ayaz-diyar-y%C3%BCce/" className="
            flex items-center justify-center size-10 bg-[#39393e] rounded-xl 
            transition duration-300 hover:rotate-10 hover:scale-[1.05]
          ">
            <svg xmlns="http://www.w3.org/2000/svg" className="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path><rect width="4" height="12" x="2" y="9"></rect><circle cx="4" cy="4" r="2"></circle></svg>
          </a>

          <a href="https://github.com/diyaryuce" className="
            flex items-center justify-center size-10 bg-[#39393e] rounded-xl 
            transition duration-300 hover:rotate-10 hover:scale-[1.05]
          ">
            <svg xmlns="http://www.w3.org/2000/svg" className="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"></path><path d="M9 18c-4.51 2-5-2-7-2"></path></svg>
          </a>

          <a href="" className="
            flex items-center justify-center size-10 bg-[#39393e] rounded-xl 
            transition duration-300 hover:rotate-10 hover:scale-[1.05]
          ">
            <img src="/img/x.png" className="size-5" />
          </a>

          <a href="" className="
            flex items-center justify-center size-10 bg-[#39393e] rounded-xl 
            transition duration-300 hover:rotate-10 hover:scale-[1.05]
          ">
            <svg xmlns="http://www.w3.org/2000/svg" className="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect width="20" height="20" x="2" y="2" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" x2="17.51" y1="6.5" y2="6.5"></line></svg>
          </a>
        </div>
      </div>
    </section>
  </>)
}